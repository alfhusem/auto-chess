...
public static class PathFinding2D
{
    ...
    {
        AutoChessController control = new AutoChessController();
        ...
        {
            //TODO dont add neighbors outside map
            int a = pos.y % 2 != 0 ? 1 : -1;
            var neighbors = new List<Vector2Int>();
            neighbors.Add(new Vector2Int(pos.x + a, pos.y + a));
            neighbors.Add(new Vector2Int(pos.x + a, pos.y));
            neighbors.Add(new Vector2Int(pos.x + a, pos.y - a));
            neighbors.Add(new Vector2Int(pos.x, pos.y - a));
            neighbors.Add(new Vector2Int(pos.x - a, pos.y));
            neighbors.Add(new Vector2Int(pos.x, pos.y + a));

            Debug.Log("NEIGHBORS of " + pos);
            foreach (Vector2Int ne in neighbors) {
                Debug.Log("-" + ne);
            }
            /*
            foreach (var neig in neighbors) {
                control.placeUnit(new Unit(neig, 2));
            }*/

            return neighbors;
        };
        Debug.Log("findX " +from + to + map.Count);
